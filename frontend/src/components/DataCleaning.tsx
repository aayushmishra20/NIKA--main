...
import { ColumnInfo, DataSummary } from "../types";
...
const isNullish = (v: any): boolean => v === null || v === undefined || v === "" ||  v === 'null';
...
const DataCleaning: React.FC = () => {
  ...
  // -------------------------------------------------------------
  const { dataset: originalDataset, updateCleanedData } = useDataContext() as {
  ...
  // State for row limit in preview
  ...
  console.log('🔄 Applying cleaning changes...');
  console.log('📊 Original dataset rows:', originalDataset.data.length);
  console.log('📊 Original dataset columns:', originalDataset.columns.length);

  // Convert preview data to the format expected by updateCleanedData
  ...
  // Find any dropped columns from DROP_COLUMN actions
  ...
  // Get the final list of columns, excluding any that were dropped
  const nextColumns = originalDataset.columns.filter(
    col => !droppedColumns.has(col.name)
  );

  console.log('📊 Cleaned data rows:', finalCleanedData.length);
  console.log('📊 Cleaned columns:', nextColumns.length);
  console.log('📊 Actions applied:', actions.length);

  // Call updateCleanedData with both the cleaned data and new columns
  updateCleanedData(finalCleanedData, nextColumns);
...
}, [preview, originalDataset, updateCleanedData, actions]);

  const handleReset = useCallback(() => {
    setActions([]);
    toast.success("Cleaning recipe cleared!");
  }, []);
  ...
  if (!originalDataset) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-10">
        ...
        >
          <CleaningServices className="h-20 w-20 text-blue-400 mx-auto mb-6" />
          <h2 className="text-2xl font-bold text-white mb-4">No Dataset Loaded</h2>
          <p className="text-gray-400 mb-6">
            Please upload a dataset first to use the Data Cleaning features.
          </p>
          <p className="text-sm text-gray-500">
            Go back to the Overview tab and upload a CSV, Excel, or JSON file to get started.
          </p>
        </motion.div>
      </div>
    );
      ...
      >
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <select 
              ...
              className="bg-gray-800 border border-gray-700 text-white text-sm rounded px-2 py-1 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
        ...
        <div className="overflow-auto max-h-[600px] rounded-lg border border-gray-700 custom-scroll relative shadow-inner">
          <table className="w-full text-sm text-left text-gray-300">
            ...
            <tbody>
              {preview.data.slice(0, rowLimit === -1 ? undefined : rowLimit).map((row, rowIndex) => (
